...
moduleBeingDefined.exportHooks[#moduleBeingDefined.exportHooks+1] = function()
	...
	-- these values are all right-aligned, so we pad left for anything longer than 1
	txt_BINGO 		= padLeft(ifei.txt_BINGO, 5)
	txt_CLOCK_H		= padLeft(ifei.txt_CLOCK_H, 2)
	txt_CLOCK_M		= padLeft(ifei.txt_CLOCK_M, 2)
	txt_CLOCK_S		= padLeft(ifei.txt_CLOCK_S, 2)
	txt_TIMER_H		= padLeft(ifei.txt_TIMER_H, 2)
	txt_TIMER_M		= padLeft(ifei.txt_TIMER_M, 2)
	txt_TIMER_S		= padLeft(ifei.txt_TIMER_S, 2)
	...
	txt_FF_L		= padLeft(ifei.txt_FF_L, 3)
	txt_FF_R		= padLeft(ifei.txt_FF_R, 3)
	txt_FUEL_DOWN	= padLeft(ifei.txt_FUEL_DOWN, 6)
	txt_FUEL_UP		= padLeft(ifei.txt_FUEL_UP, 6)
	txt_OilPress_L	= padLeft(ifei.txt_OilPress_L, 3)
	txt_OilPress_R	= padLeft(ifei.txt_OilPress_R, 3)
	txt_RPM_L		= padLeft(ifei.txt_RPM_L, 3)
	txt_RPM_R		= padLeft(ifei.txt_RPM_R, 3)
	txt_TEMP_L		= padLeft(ifei.txt_TEMP_L, 3)
	txt_TEMP_R		= padLeft(ifei.txt_TEMP_R, 3)
	txt_Codes		= padLeft(ifei.txt_Codes, 3)
	txt_SP			= padLeft(ifei.txt_SP, 3)
	txt_TimeSetMode	= padLeft(ifei.txt_TimeSetMode, 6)
	txt_T			= padLeft(ifei.txt_T, 6)
