...
It provides a stable, documented interface for external hardware and software to interact with the clickable cockpit of a DCS: World aircraft.
  ...
  Info: Since the Website is down, every information is included in the Repositorie here.
  Also add the arduino-library and com-handler to save it, if the also get lost. 
  But we will not maintain it at this time.
