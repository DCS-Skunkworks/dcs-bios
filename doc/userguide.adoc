...
DCS BIOS will not let that happen.  In fact, DCS-BIOS was written just for that reason.  DCS BIOS will isolate your panels from virtually any changes that Eagle Dynamics might introduce in an update.
...
DCS BIOS is the most comprehensive means of connecting all DCS aircraft buttons, switches, rotary encodes, potentiometers, push buttons, LED Indciators, displays, three way switches etc., etc.  In sort There is nothing in the cockpit that DCS BIOS does not cover.    
