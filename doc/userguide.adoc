...
* take a few minutes and have a look at the Arduino board you are using.  It has a  bunch of holes that you can plug wires into.  They are all labeled.  If I were you I would spend a few minutes on the Arduino web site and do a bit of reading on the basics of using Arduino boards or maybe look at a couple of the intro YouTube videos. I'm sure you took some time to read up on how to fly an A10C so take a bit of time now and read a little on Arduino. It will help you as you move forward to more complex switch panels. Enough rambling and on to the first step.  Connect your push button switch to your Arduino board at pin 10 and pin GND (ground).  There are several grounds on the board and they are all connected together so it doesn't matter which one you choose.
...
Each program (sketch) you create will use a basic template that we have provided. This template automatically reads the state of your connected switches and reports this to DCS-BIOS which then updates the switches of your DCS aircraft.  This ensures that whatever you switches are set to, you aircraft will be synced to them.
...
NOTE: Arduino programs are called "sketches".
...
. Go to your saved Games folder/DCS-BIOS and double-click on the file `protocol-reference.html` which is located in the `doc` folder in your DCS-BIOS download.  You web browsers will automatically open and either display all sorts of colorful headings and code or you will see a red warning followed by a bunch of nonsense.  In that case you need to enable JavaScript in your web browser and reload the page.  There is usually a box at the bottom of your screen that appears and asks you if you want to display "active content".  You need to say yes.
