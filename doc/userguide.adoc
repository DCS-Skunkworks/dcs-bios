...
DCS-BIOS provides you with a library of components that include ALL DCS aircraft buttons, switches, rotary encodes, potentiometers, push buttons, LED Indciators, displays, three way switches etc., etc.  In sort There is nothing in the cockpit that DCS-BIOS does not cover.    
...
* An Arduino board ( the Arduino "UNO" is the suggested Beginners board)
* A push button
* An LED
...
. Download the latest release of DCS-BIOS from https://github.com/jboecker/dcs-bios/releases[GitHub] and extract the ZIP file somewhere on your hard drive
...
----
This should take you to your DCS configuration folder (its has several subfolders including `Logs` and `Missions`).  You are looking for the `scripts folder`.  (Not all configurations have a scripts folder and do not be concerned if you don`t have one.  Just follow these steps.)
...
Once, Notepad++ is installed you need to open your existing `Export.lua` file and copy the following line shown below and paste it into notepad++.  It should be placed as the last line in the export.lua file. :

[source,lua]
----
dofile(lfs.writedir()..[[Scripts\DCS-BIOS\BIOS.lua]])
----
Once you have completed that small change, save the file.  That is the only change you need to make.  Now we move on to the good stuff...connecting switches and LED's.
...
* Connect your LED and a 220 ohm 1/4 watt current limiting resistor to the Arduino board at pin 13 and pin GND (ground).
...
Each program you create will use a basic template that we have provided. This template automatically reads the state of your connected switches and reports this to DCS-BIOS which then updates the switches of your DCS aircraft.  This ensures that whatever your switches say, your aircraft will be synced to them.
...
To complete the template program, you are going to copy lines of code called "snippets" from the reference documentation.  Remember this term "snippets".  It will be used frequently throughout the Users Guide.
...
Now its time to open Notepad++ again.  This time your going to open `connect-serial-port.cmd` from your DCS-BIOS download.  Locate the line that starts with `COMPORT=`.
Replace the existing number with the COM port number you used to program your Arduino board and save the file.  When done, save the file.
...
You can make any panel you like using the standard template that we provided as the outline for your sketch.  Its just a matter of adding the lines of code from the reference documentation for the switches you wish to use and connecting the appropriate switch type to the appropriate pin number on the Arduino board.  If your intent is to also add displays you will need to have additional knowledge working with Arduino. There are so many type of displays that it is beyond the scope of the Users Manual to give instruction on every type of display thats available. We have provided the basic structure for a novice to create a switch panel using the Arduino platform with very little further knowledge of Arduino, DCS or lua.  
...
It will cover all of the different control types that are supported by the Arduino library.  The information provided at this point will require a high level of knowledge with Arduino and lua coding.
